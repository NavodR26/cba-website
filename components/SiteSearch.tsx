...
export default function SiteSearch({ showTrigger = false, triggerClassName }: SiteSearchProps) {
  ...
  const searchOverlay = open ? (
    <div className="fixed inset-0 z-[90]">
      ...
      />
      <div
        className="relative mx-auto w-[calc(100%-24px)] max-w-2xl overflow-hidden rounded-2xl border border-white/40 bg-white shadow-2xl"
        style={{ marginTop: 'max(5rem, env(safe-area-inset-top))' }}
      >
        ...
        <div className="max-h-[min(56dvh,28rem)] overflow-y-auto p-2">
          {results.length === 0 ? (
    ...
    <>
      {showTrigger && (
        <button
          ...
          className={
            triggerClassName ??
            'inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/12 backdrop-blur-[10px] px-3 py-2 text-xs font-semibold shadow-sm transition hover:border-[var(--maroon)]/30'
          }
      ...
      {mounted && searchOverlay ? createPortal(searchOverlay, document.body) : null}
    </>
  )
}
